class Affector extends Draggable {
    ...
    draw() {
        if(this.draggable()) {
            ...
            textFont("monospace")
            textSize(10)
            text(this.registry.get("force").value < 0 ? `Attractor [${this.id}]` : this.registry.get("force").value > 0 ? `Repeller [${this.id}]` : `Affector [${this.id}]`, mouseX + 13, mouseY)
        }

        if(this.registry.get("force").value < 0) stroke(255, 205)
        else if(this.registry.get("force").value > 0) stroke(0, 205)
        else stroke(255, 0, 255, 205)
